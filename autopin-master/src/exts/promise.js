    ...
    if (shouldRetry && anyRetriesLeft) {
      ...
      if (config.delay) {
        ...
        if (config.messageFormatter) {
          console.warn(config.messageFormatter(thisRetryDelay))
        }
...
Promise.tryUntil = function tryUntil(...args) {
  ...
  let normalizedConfig = {
    ...
    messageFormatter: userConfig.messageFormatter,
  };

  return attempt(normalizedConfig, promiseFunc, funcArgs);
};

Promise.bluebird = function(sourcePromise) {
  return new Promise((resolve, reject) => sourcePromise.then(resolve, reject));
};
